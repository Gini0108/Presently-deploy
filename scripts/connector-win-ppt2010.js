...
var cmdSTAT = function () {
  ...
  var state =
    (ss !== null
      ? "viewing"
      : (pres !== null ? "editing" : (app !== null ? "started" : "closed")));
  return '{ "response": { ' +
    '"state": "' + state + '", ' +
    '"position": ' + slideCur + ", " +
    '"slides": ' + slideMax + " " +
...
var cmdINFO = function () {
  ...
  var pres = activePresentation(app);
  if (pres === null) {
    throw new Error("There is no presentation loaded");
  }
  var titles = "";
  var notes = "";
  ...
  for (var i = 1; i <= slides.Count; i++) {
    ...
    for (j = 1; j <= shapes.Count; j++) {
      ...
      if (shape.Type === 14 /* msoPlaceholder */) {
        if (
          shape.PlaceholderFormat.Type === 1 /* ppPlaceholderTitle */ ||
...
var cmdCONTROL = function (cmd, arg) {
  ...
  if (cmd.match(/(FIRST|LAST|GOTO|PREV|NEXT)$/)) {
    if (ss === null) {
...
while (!WScript.StdIn.AtEndOfStream) {
  ...
  try {
    if (cmd === "STAT") {
      ...
      cmd.match(/^(BOOT|QUIT|OPEN|CLOSE|START|STOP|FIRST|LAST|GOTO|PREV|NEXT)$/)
    ) {
